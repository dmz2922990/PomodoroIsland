...
**通知示例**：AI 调用 `ask_user` → 岛屿自动弹出选项卡片 → 你点了某个选项 → AI 收到 `{"status":"answered","selected":["休息一下"]}`。来源识别：初始化握手 `clientInfo.name` 或调用参数 `source`，卡片带来源徽标；单来源最多 2 条待响应、全局最多 3 条。多条通知同时待处理时纵向堆叠：提问卡（橘黄圆角边框）先到在前、每张可直接作答，被动通知折叠为底部紧凑行。
